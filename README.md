...
I tried to reduce the jitter within the ROI boxes that's caused by the fluctuating color detection by averaging each detected box over its x predecessors, but I found that it's not that easy to keep track of these regions. Since the jittering didn't turn out to be a big problem because the detection is generally quite stable, I scrapped the idea of a "video stabilizer" and just continued with the "unstable" implementation.  
